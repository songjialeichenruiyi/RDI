...
# 定义模型、损失函数和优化器
model = ModifiedResNet50(num_classes=200).to(device)
...
scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.5)  # 每5个epoch学习率衰减为原来的一半
...
# 加载模型并测试
loaded_model =  ModifiedResNet50(num_classes=200).to(device)
