...
model_save_path = 'boundary_robustness/models/cifar100/DenseNet121_cifar100.pth'
# 加载模型参数
...
# 创建对抗数据集
trainset = AdversarialDataset(adv_data_file="boundary_robustness/attackData/Cifar100/DenseNet121_datas.npy", label_file="boundary_robustness/attackData/Cifar100/DenseNet121_labels.npy", transform=transform)
adv_train_loader = DataLoader(trainset, batch_size=128, shuffle=True)
...
def mixed_train_loader(original_loader, adv_loader):
    ...
    for (orig_images, orig_labels), (adv_images, adv_labels) in zip(original_loader, adv_loader):
        ...
        # 从对抗样本中取全部
        adv_images_all = adv_images[: 2 * min_batch_size]
        adv_labels_all = adv_labels[: 2 * min_batch_size]
...
optimizer = optim.SGD(model.parameters(), lr=0.005, momentum=0.6)
...
# 保存模型
model_save_path2 = 'boundary_robustness/models/cifar100/AdvDenseNet121_cifar100.pth'
