...
# 加载torchvision.models中的AlexNet模型，并修改输入层以适应CIFAR-10数据集
class ModifiedAlexNet(nn.Module):
    def __init__(self, num_classes=10):
        super(ModifiedAlexNet, self).__init__()
        self.model = models.alexnet(weights = None)
        self.model.features[0] = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1)  # 修改输入层以适应CIFAR-10
        self.model.classifier[6] = nn.Linear(4096, num_classes)  # 修改输出层以适应CIFAR-10的类别数量
...
model_save_path = 'boundary_robustness/models/cifar10/AlexNet_cifar10_2.pth'
# 加载模型参数
...
# 创建对抗数据集
trainset = AdversarialDataset(adv_data_file="boundary_robustness/attackData/Cifar10/AlexNet_datas.npy", label_file="boundary_robustness/attackData/Cifar10/AlexNet_labels.npy", transform=transform)
