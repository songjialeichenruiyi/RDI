...
# 修改 torchvision.models 中的 DenseNet121 模型，为Tiny-ImageNet数据集调整输出层
class ModifiedDenseNet121(nn.Module):
    def __init__(self, num_classes=200):  # Tiny-ImageNet类别数为 200
        super(ModifiedDenseNet121, self).__init__()
        self.model = models.densenet121(weights = None)  # 使用预训的 DenseNet121
        # 修改第一层卷积来适应Tiny-ImageNet小尺寸输入
        self.model.features.conv0 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
        # 禁用最大池化层，避免过度下采样对Tiny-ImageNet小尺寸输入的影响
...
model_save_path = 'boundary_robustness/models/imageNet/DenseNet121_imageNet.pth'
...
if torch.cuda.is_available():
    model = nn.DataParallel(model, device_ids=[0, 1, 2, 3])  # 使用四张 GPU
...
# 创建对抗数据集
trainset = AdversarialDataset(adv_data_file="boundary_robustness/attackData/ImageNet/DenseNet121_datas.npy", label_file="boundary_robustness/attackData/ImageNet/DenseNet121_labels.npy", transform=transform)
...
# 保存模型
model_save_path2 = 'boundary_robustness/models/imageNet/AdvDenseNet121_imageNet.pth'
