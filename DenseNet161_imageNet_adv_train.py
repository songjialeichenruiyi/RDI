...
# 修改 torchvision.models 中的 DenseNet161 模型，为TinyImageNet数据集调整输出层
class ModifiedDenseNet161(nn.Module):
    def __init__(self, num_classes=200):  # TinyImageNet类别数为 100
        super(ModifiedDenseNet161, self).__init__()
        self.model = models.densenet161(weights = None)  # 使用预训练的 DenseNet161
        # 修改第一层卷积来适应 TinyImageNet 小尺寸输入
        self.model.features.conv0 = nn.Conv2d(3, 96, kernel_size=3, stride=1, padding=1, bias=False)
        # 禁用最大池化层，避免过度下采样对 TinyImageNet 小尺寸输入的影响
...
# model_save_path = 'boundary_robustness/models/imageNet/DenseNet161_imageNet.pth'
# 加载模型参数
# model.load_state_dict(torch.load(model_save_path))

# 1. 加载训练好的模型
model = ModifiedDenseNet161()
model_save_path = 'boundary_robustness/models/imageNet/DenseNet161_imageNet.pth'
...
if torch.cuda.is_available():
    model = nn.DataParallel(model, device_ids=[1, 2, 3])  # 使用四张 GPU
    # model = model.cuda()  # 将模型迁移到 GPU
    model = model.to('cuda:1')  # 显式将模型迁移到 cuda:1，确保主设备为 cuda:1
...
# 创建对抗数据集
trainset = AdversarialDataset(adv_data_file="boundary_robustness/attackData/ImageNet/DenseNet161_datas.npy", label_file="boundary_robustness/attackData/ImageNet/DenseNet161_labels.npy", transform=transform)
adv_train_loader = DataLoader(trainset, batch_size=160, shuffle=True, num_workers=4)
...
train_loader = DataLoader(train_dataset, batch_size=160, shuffle=True, num_workers=4)
...
optimizer = optim.SGD(model.parameters(), lr=0.0006, momentum=0.5, weight_decay=1e-4)
...
# 保存模型
model_save_path2 = 'boundary_robustness/models/imageNet/AdvDenseNet161_imageNet.pth'
torch.save(model.state_dict(), model_save_path2)
print(f'Model saved to {model_save_path2}')


# 创建新的MobileNetV2模型实例
loaded_model = ModifiedDenseNet161().to(device)
